...
def get_voucher_keyboard():
    ...
    keyboard.add(
        ...
        InlineKeyboardButton("🔤+🔢 MIXED 7လုံး", callback_data="scan_mixed7"),   # <-- Added
...
Key ရရှိပြီးပါက PAID USER ဖြစ်ရန် နှိပ်ပါ"""
    ...
    if call.data.startswith("scan_"):
        ...
        # For digit modes (6,7,8,9) show digit selection
        if mode in ["6", "7", "8", "9"]:
            await bot.edit_message_text(
                chat_id=chat_id,
                message_id=call.message.message_id,
        ...
        # For other modes (including mixed7) just set and show start button
...
def iter_codes(mode, start_digit=None):
    ...
    if mode in ["6", "7", "8", "9"]:
        ...
        if start_digit is not None:
            # start_digit is a string like "3", we use it to limit range
            try:
                start_int = int(start_digit)
                # For 6,7,8,9 we only consider codes starting with that digit
                # Generate all numbers from start_int * 10^(length-1) to (start_int+1)*10^(length-1)-1
                start = start_int * (10 ** (length - 1))
                end = (start_int + 1) * (10 ** (length - 1))
                # But if we generate all, it could be huge (for 9 it's 100 million). We'll use random sampling instead.
                # Better: use random.randint for infinite generator with start_digit filter
                # But to keep it simple and efficient, we'll generate random numbers within that range
                # and yield them one by one (infinite, but with start_digit constraint)
                while True:
                    code_num = random.randint(start, end - 1)
                    code_str = str(code_num).zfill(length)
                    yield code_str
            except ValueError:
                # if start_digit is not a valid digit, fallback to full random
                while True:
                    yield digit_generator(length)
        else:
            # No start_digit: infinite random generator (much faster than shuffling all)
            while True:
                yield digit_generator(length)
...
async def run_bruteforce(mode, chat_id, session_url, scan_id, message=None, progress_msg=None, start_digit=None):
    ...
    # For numeric modes, we can optionally set a total for progress, but since we use infinite generator, total=None
    total = None  # all modes are infinite now, we don't track total
    ...
    try:
        while True:
            ...
            # Always show checked count, found, speed (total is None)
